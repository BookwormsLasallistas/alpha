...
if ($disponibilidad != "all" || $lenguaje != "all" || $editorial != "all" || $genero != "all" || $calificacion != "all") {
    ...
    $query_1="SELECT * FROM libros WHERE ";
    if ($disponibilidad != "all") {
    ...
    $query = "SELECT * FROM libros WHERE nombre LIKE '%$buscar%' OR autor LIKE '%$buscar%' OR genero LIKE '%$ buscar%' OR editorial LIKE '%$buscar%'";
    ...
    $query_1="SELECT * FROM libros WHERE nombre LIKE '%$buscar%' OR autor LIKE '%$buscar%' OR genero LIKE '%$buscar%' OR editorial LIKE '%$buscar%' LIMIT $inicio, $rows_por_pagina";
    $resultado_limite = mysqli_query($conn, $query_1);
    ...
    $query_1="SELECT * FROM libros LIMIT $inicio, $rows_por_pagina";
    $resultado_limite = mysqli_query($conn, $query_1);
}
...
if (!isset($_SESSION)) {
    ...
    /* ------------------------------ Guardar libro en carrito --------------------------------------------- */
    if (isset($_GET['id'])) {
            $libroId = $_GET["id"];
            $usuarioId = $_SESSION['id_usuario'];

            $qry2 ="INSERT INTO carrito (id_usuario, id_libro) VALUES ($usuarioId, $libroId)";
                ...
                if ($resultado2) {
                        ...
                        Swal.fire({
                         background: "white", // Fondo transparente
                            ...
                            window.location.href = "libros.php"; // Redirige al usuario después de cerrar el mensaje
                            }
                        });
                        });
                        </script>';
...
<div class="bg"></div>
<h1 class="titulo-pagina">¡Bienvenido a la biblioteca!</h1>
<br><br>

<!-- Agrega un contenedor para los select de filtro -->
<div>
    <div id="filters" class="filterBox">
    <form id="filtro-form" action="../paginas/libros.php" method="get">
                ...
                <div class="container">
                    ...
                    <div class="bottom">
                        <div class="left">
                            ...
                            ?>
                                <a title="¡Añádeme a tus favoritos!" href="libros.php?id=<?php echo $row['id']; ?>"><i id="icon"  class="fa-solid fa-bookmark fa-xl xd" style="color: #e7b11f;"></i></a>
                            <?php    
                                    }
                                }
                            ?>
